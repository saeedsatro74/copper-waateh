...
interface DeductCoilModalProps {
  coil: CoilItem | null;
  isOpen: boolean;
  onClose: () => void;
...
export const DeductCoilModal: React.FC<DeductCoilModalProps> = ({
  coil,
  isOpen,
  onClose,
  onConfirm,
  ...
  useEffect(() => {
    if (coil && mode === 'meters') {
      const weightPerMeter = estimateCopperWeightPerMeter(coil.diameter, coil.thickness);
  ...
  }, [coil, mode, metersInput]);

  if (!isOpen || !coil) return null;

  const weightPerMeter = estimateCopperWeightPerMeter(coil.diameter, coil.thickness);
  const deductKg = parseFloat(deductKgInput) || 0;
  const remainingWeight = Math.max(0, coil.weightKg - deductKg);
      ...
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="bg-amber-600 text-white p-5 flex items-center justify-between">
          <div className="flex items-center space-x-2 space-x-reverse">
            ...
            <h3 className="font-bold text-base">برداشت از کلاف و انتقال به خورده</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-right">
          ...
          <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 space-y-1.5 text-xs">
            <div className="flex justify-between font-bold text-slate-900">
              <span>کلاف: {coil.code}</span>
              <span className="text-amber-700">برند {coil.brand}</span>
            </div>
            <div className="text-slate-600">
              سایز: {coil.thickness}mm / {coil.diameter}
            </div>
            <div className="text-slate-900 font-bold pt-1 border-t border-slate-200">
              وزن کامل کلاف: {formatKg(coil.weightKg)}
            </div>
          </div>
          ...
          {mode === 'meters' ? (
            <div className="space-y-3">
              ...
              <div className="p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs flex items-center justify-between">
                <span className="text-slate-500">تخمین وزن هر متر (سایز {coil.diameter}):</span>
          ...
          <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-xl text-xs space-y-1.5">
            ...
            {remainingWeight > 0 && (
              <p className="text-[11px] text-amber-800 pt-1 font-medium leading-relaxed">
                ⚡ باقیمانده کلاف ({formatKg(remainingWeight)}) به طور خودکار به دسته <b>خورده‌فروشی</b> منتقل خواهد شد.
              </p>
            )}
          </div>
          ...
          <div className="pt-2 flex justify-end space-x-2 space-x-reverse">
            ...
            <button
              type="submit"
              disabled={deductKg <= 0 || deductKg > coil.weightKg}
